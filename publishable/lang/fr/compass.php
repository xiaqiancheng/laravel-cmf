<?php

return [
    'welcome'                => 'Bienvenue sur LaravelCmf Compass. Chaque bonne nouvelle application ont besoin d\'une boussole pour se diriger dans la bonne direction.
    Dans cette section, vous trouverez plusieurs ressources et tâches administrative afin de vous guider lors de la création de votre application',
    'links'         => [
        'title'                 => 'Liens',
        'documentation'         => 'Documentation',
        'laravel_cmf_homepage'      => 'LaravelCmf Homepage',
        'laravel_cmf_hooks'         => 'LaravelCmf Hooks',
    ],
    'commands'      => [
        'title'                 => 'Commandes',
        'text'                  => 'Lancer les Commandes artisan depuis LaravelCmf.',
        'clear_output'          => 'Vider la sortie',
        'command_output'        => 'Sortie des commandes Artisan',
        'additional_args'       => 'Arguments additionels?',
        'run_command'           => 'Lancer la commande',
    ],
    'resources'     => [
        'title'                 => 'Resources',
        'text'                  => 'Ressources LaravelCmf pour vous aider a rechercher certaines chose plus vite.',

    ],
    'logs'          => [
        'title'                 => 'Logs',
        'text'                  => 'Les logs de votre app',
        'file_too_big'          => 'Fichier de log >50M, veuillez le télécharger.',
        'level'                 => 'Niveau',
        'context'               => 'Contexte',
        'date'                  => 'Date',
        'content'               => 'Contenu',
        'download_file'         => 'Télécharger le fichier',
        'delete_file'           => 'Supprimer le fichier',
        'delete_all_files'      => 'Supprimer tout les fichiers',
        'delete_success'        => 'Fichier log supprimé avec succès:',
        'delete_all_success'    => 'Tout les fichiers de log ont été supprimés avec succès',

    ],
    'fonts'         => [
        'title'                 => 'Police de caractère',
        'font_class'            => 'Classe de correspondance à la police LaravelCmf',
        'font_character'        => 'Charactère de correspondance à la police LaravelCmf',
    ],
];
