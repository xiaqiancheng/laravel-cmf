...
class MenuTest extends TestCase
{
    ...
    public function testCanRenameMenu()
    {
        ...
        $this->visitRoute('laravel-cmf.menus.edit', ['id' => $menu->id])
             ->seeInField('name', $menu->name)
             ->type('new_admin', 'name')
             ->seeInElement('button', __('laravel-cmf::generic.save'))
             ->press(__('laravel-cmf::generic.save'))
             ->seePageIs(route('laravel-cmf.menus.index'))
             ->seeInDatabase('menus', [
                'id'   => $menu->id,
                'name' => 'new_admin',
             ]);
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $this->delete(route('laravel-cmf.menus.item.destroy', [
            'menu' => $menu->id,
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $this->post(route('laravel-cmf.menus.item.add', [
            'menu'    => $menu->id,
            'menu_id' => $menu->id,
            'type'    => 'url',
            'color'   => '#000000',
    ...
    public function testCanUpdateMenuItem()
    {
        ...
        $this->put(route('laravel-cmf.menus.item.update', [
            'id'      => $item->id,
            'menu'    => $menu->id,
            'type'    => 'url',
            'color'   => '#000000',
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $response = $this->call('GET', route('laravel-cmf.menus.builder', ['menu' => $menu->id]));
