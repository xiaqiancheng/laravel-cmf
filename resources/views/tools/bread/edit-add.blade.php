...
@if (isset($dataType->id))
    @section('page_title', __('laravel-cmf::bread.edit_bread_for_table', ['table' => $dataType->name]))
@else
    @section('page_title', __('laravel-cmf::bread.create_bread_for_table', ['table' => $table]))
@endif

@section('page_header')
    <div class="page-title">
        <i class="laravel-cmf-data"></i>
        @if (isset($dataType->id))
            {{ __('laravel-cmf::bread.edit_bread_for_table', ['table' => $dataType->name]) }}
        @else
            {{ __('laravel-cmf::bread.create_bread_for_table', ['table' => $table]) }}
        @endif
    </div>
    @php
    ...
    @endphp
    @include('laravel-cmf::multilingual.language-selector')
@stop
...
<ol class="breadcrumb hidden-xs">
    ...
    <li class="active">
        <a href="{{ route('laravel-cmf.bread.index') }}">
            {{ __('laravel-cmf::generic.bread') }}
        </a>
    </li>
    <li class="active">
        @if(isset($dataType->id))
        <a href="{{ route('laravel-cmf.bread.edit', $table) }}">
            {{ $dataType->display_name_singular }}
        </a>
        @else
        <a href="{{ route('laravel-cmf.bread.create', ['name' => $table]) }}">
            {{ $display_name }}
        </a>
        @endif
    </li>
    <li>
        {{ isset($dataType->id) ? __('laravel-cmf::generic.edit') : __('laravel-cmf::generic.add') }}
    </li>
</ol>
@endsection

@section('content')
    <div class="page-content container-fluid" id="laravelCmfBreadEditAdd">
        <div class="row">
            <div class="col-md-12">

                <form action="@if(isset($dataType->id)){{ route('laravel-cmf.bread.update', $dataType->id) }}@else{{ route('laravel-cmf.bread.store') }}@endif"
                ...
                @endif
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="laravel-cmf-bread"></i> {{ ucfirst($table) }} {{ __('laravel-cmf::bread.bread_info') }}</h3>
                            <div class="panel-actions">
                                <a class="panel-action laravel-cmf-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="name">{{ __('laravel-cmf::database.table_name') }}</label>
                            ...
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="display_name_singular">{{ __('laravel-cmf::bread.display_name_singular') }}</label>
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('laravel-cmf::bread.display_name_singular') }}"
                                           value="{{ $dataType->display_name_singular ?? $display_name }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="display_name_plural">{{ __('laravel-cmf::bread.display_name_plural') }}</label>
                                    @if($isModelTranslatable)
                                        @include('laravel-cmf::multilingual.input-hidden', [
                                            'isModelTranslatable' => true,
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('laravel-cmf::bread.display_name_plural') }}"
                                           value="{{ $dataType->display_name_plural ?? $display_name_plural }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="slug">{{ __('laravel-cmf::bread.url_slug') }}</label>
                                    <input type="text" class="form-control" name="slug" placeholder="{{ __('laravel-cmf::bread.url_slug_ph') }}"
                                           value="{{ $dataType->slug ?? $slug }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="icon">{{ __('laravel-cmf::bread.icon_hint') }} <a
                                                href="{{ route('laravel-cmf.compass.index', [], false) }}#fonts"
                                                target="_blank">{{ __('laravel-cmf::bread.icon_hint2') }}</a></label>
                                    <input type="text" class="form-control" name="icon"
                                           placeholder="{{ __('laravel-cmf::bread.icon_class') }}"
                                           value="{{ $dataType->icon ?? '' }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="model_name">{{ __('laravel-cmf::bread.model_name') }}</label>
                                    <span class="laravel-cmf-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('laravel-cmf::bread.model_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="model_name" placeholder="{{ __('laravel-cmf::bread.model_class') }}"
                                           value="{{ $dataType->model_name ?? $model_name }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="controller">{{ __('laravel-cmf::bread.controller_name') }}</label>
                                    <span class="laravel-cmf-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('laravel-cmf::bread.controller_name_hint') }}"></span>
                                    <input type="text" class="form-control" name="controller" placeholder="{{ __('laravel-cmf::bread.controller_name') }}"
                                           value="{{ $dataType->controller ?? '' }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="policy_name">{{ __('laravel-cmf::bread.policy_name') }}</label>
                                    <span class="laravel-cmf-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('laravel-cmf::bread.policy_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="policy_name" placeholder="{{ __('laravel-cmf::bread.policy_class') }}"
                                           value="{{ $dataType->policy_name ?? '' }}">
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="generate_permissions">{{ __('laravel-cmf::bread.generate_permissions') }}</label><br>
                                    ...
                                    <input type="checkbox" name="generate_permissions" class="toggleswitch" data-on="{{ __('laravel-cmf::generic.yes') }}" data-off="{{ __('laravel-cmf::generic.no') }}"
                                           @if($checked) checked @endif>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="server_side">{{ __('laravel-cmf::bread.server_pagination') }}</label><br>
                                    <?php $checked = (isset($dataType->server_side) && $dataType->server_side == 1) ? true : (isset($server_side) && $server_side) ? true : false; ?>
                                    <input type="checkbox" name="server_side" class="toggleswitch" data-on="{{ __('laravel-cmf::generic.yes') }}" data-off="{{ __('laravel-cmf::generic.no') }}"
                                           @if($checked) checked @endif>
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-3 form-group">
                                    <label for="order_column">{{ __('laravel-cmf::bread.order_column') }}</label>
                                    <span class="laravel-cmf-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('laravel-cmf::bread.order_column_ph') }}"></span>
                                    <select name="order_column" class="select2 form-control">
                                          <option value="">-- {{ __('laravel-cmf::generic.none') }} --</option>
                                ...
                                <div class="col-md-3 form-group">
                                    <label for="order_display_column">{{ __('laravel-cmf::bread.order_ident_column') }}</label>
                                    <span class="laravel-cmf-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('laravel-cmf::bread.order_ident_column_ph') }}"></span>
                                ...
                                <div class="col-md-3 form-group">
                                    <label for="order_direction">{{ __('laravel-cmf::bread.order_direction') }}</label>
                                    <select name="order_direction" class="select2 form-control">
                                        <option value="asc" @if(isset($dataType) && $dataType->order_direction == 'asc') selected @endif>
                                            {{ __('laravel-cmf::generic.ascending') }}
                                        </option>
                                        <option value="desc" @if(isset($dataType) && $dataType->order_direction == 'desc') selected @endif>
                                            {{ __('laravel-cmf::generic.descending') }}
                                        </option>
                                    </select>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="default_search_key">{{ __('laravel-cmf::bread.default_search_key') }}</label>
                                    <span class="laravel-cmf-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('laravel-cmf::bread.default_search_key_ph') }}"></span>
                            ...
                            <div class="row clearfix">
                                @if (isset($scopes) && isset($dataType))
                                    <div class="col-md-3 form-group">
                                        <label for="scope">{{ __('laravel-cmf::bread.scope') }}</label>
                                ...
                                <div class="col-md-9 form-group">
                                    <label for="description">{{ __('laravel-cmf::bread.description') }}</label>
                                    <textarea class="form-control" name="description"
                                              placeholder="{{ __('laravel-cmf::bread.description') }}"
                    ...
                    <div class="panel panel-primary panel-bordered">
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="laravel-cmf-window-list"></i> {{ __('laravel-cmf::bread.edit_rows', ['table' => $table]) }}:</h3>
                            <div class="panel-actions">
                                <a class="panel-action laravel-cmf-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row fake-table-hd">
                                <div class="col-xs-2">{{ __('laravel-cmf::database.field') }}</div>
                                <div class="col-xs-2">{{ __('laravel-cmf::database.visibility') }}</div>
                                <div class="col-xs-2">{{ __('laravel-cmf::database.input_type') }}</div>
                                <div class="col-xs-2">{{ __('laravel-cmf::bread.display_name') }}</div>
                                <div class="col-xs-4">{{ __('laravel-cmf::database.optional_details') }}</div>
                            </div>
                            ...
                            @foreach($fieldOptions as $data)
                                ...
                                @if(isset($dataType->id))
                                    <?php $dataRow = LaravelCmf::model('DataRow')->where('data_type_id', '=',
                                ...
                                <div class="row row-dd">
                                    <div class="col-xs-2">
                                        ...
                                        <strong>{{ __('laravel-cmf::database.type') }}:</strong> <span>{{ $data['type'] }}</span><br/>
                                        <strong>{{ __('laravel-cmf::database.key') }}:</strong> <span>{{ $data['key'] }}</span><br/>
                                        <strong>{{ __('laravel-cmf::generic.required') }}:</strong>
                                        @if($data['null'] == "NO")
                                            <span>{{ __('laravel-cmf::generic.yes') }}</span>
                                            <input type="hidden" value="1" name="field_required_{{ $data['field'] }}"
                                                   checked="checked">
                                        @else
                                            <span>{{ __('laravel-cmf::generic.no') }}</span>
                                            <input type="hidden" value="0" name="field_required_{{ $data['field'] }}">
                                        @endif
                                        <div class="handler laravel-cmf-handle"></div>
                                    ...
                                    <div class="col-xs-2">
                                        <input type="checkbox"
                                               ...
                                               @elseif(!isset($dataRow->browse))
                                                    {{ 'checked="checked"' }}
                                               @endif>
                                        <label for="field_browse_{{ $data['field'] }}">{{ __('laravel-cmf::generic.browse') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_read_{{ $data['field'] }}">{{ __('laravel-cmf::generic.read') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_edit_{{ $data['field'] }}">{{ __('laravel-cmf::generic.edit') }}</label><br/>
                                        <input type="checkbox"
                                            ...
                                            <label for="field_add_{{ $data['field'] }}">{{ __('laravel-cmf::generic.add') }}</label><br/>
                                        <input type="checkbox"
                            ...
                            @if(isset($dataTypeRelationships))
                                @foreach($dataTypeRelationships as $relationship)
                                    @include('laravel-cmf::tools.bread.relationship-partial', $relationship)
                                @endforeach
                            @endif
                        ...
                        <div class="panel-footer">
                             <div class="btn btn-new-relationship"><i class="laravel-cmf-heart"></i> <span>
                             {{ __('laravel-cmf::database.relationship.create') }}</span></div>
                        </div>
                    </div><!-- .panel -->

                    <button type="submit" class="btn pull-right btn-primary">{{ __('laravel-cmf::generic.submit') }}</button>
...
@include('laravel-cmf::tools.bread.relationship-new-modal')
...
@section('javascript')
    ...
    <script>
            ...
            $('textarea[data-editor]').each(function () {
                ...
                textarea.closest('form').on('submit', function (ev) {
                    if (window.invalidEditors.length) {
                        ...
                        toastr.error('{{ __('laravel-cmf::json.invalid_message') }}', '{{ __('laravel-cmf::json.validation_errors') }}', {"preventDuplicates": true, "preventOpenDuplicates": true});
                    } else {
       ...
       $(function () {
            ...
            $('.laravel-cmf-relationship-details-btn').click(function(){
                $(this).toggleClass('open');
                if($(this).hasClass('open')){
                    $(this).parent().parent().find('.laravel-cmf-relationship-details').slideDown();
                } else {
                    $(this).parent().parent().find('.laravel-cmf-relationship-details').slideUp();
                }
            });
        ...
        function populateRowsFromTable(dropdown){
            ...
            if(tbl.length != 0){
                $.get('{{ route('laravel-cmf.database.index') }}/' + tbl, function(data){
                    $(dropdown).empty();
