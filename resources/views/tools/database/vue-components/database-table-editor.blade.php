...
<div class="panel panel-bordered">
    <div class="panel-body">
        ...
        @else
            <div class="col-md-6">
        @endif
                <label for="name">{{ __('laravel-cmf::database.table_name') }}</label><br>
                <input v-model.trim="table.name" type="text" class="form-control" placeholder="{{ __('laravel-cmf::database.table_name') }}" required pattern="{{ $db->identifierRegex }}">
            </div>

        @if($db->action == 'create')
            <div class="col-md-3 col-sm-4 col-xs-6">
                <label for="create_model">{{ __('laravel-cmf::database.create_model_table') }}</label><br>
                <input type="checkbox" name="create_model" data-toggle="toggle"
                       data-on="{{ __('laravel-cmf::generic.yes_please') }}" data-off="{{ __('laravel-cmf::generic.no_thanks') }}">
            </div>
            {{--
                Hide migration button until feature is available.
                 <div class="col-md-3 col-sm-4 col-xs-6">
                    <label for="create_migration">{{ __('laravel-cmf::database.create_migration') }}</label><br>
        ...
        <div v-if="compositeIndexes.length" v-once class="alert alert-danger">
            <p>{{ __('laravel-cmf::database.no_composites_warning') }}</p>
        </div>

        <div id="alertsContainer"></div>

        <template v-if="tableHasColumns">
            <p>{{ __('laravel-cmf::database.table_columns') }}</p>

            <table class="table table-bordered" style="width:100%;">
                <thead>
                <tr>
                    <th>{{ __('laravel-cmf::generic.name') }}</th>
                    <th>{{ __('laravel-cmf::generic.type') }}</th>
                    <th>{{ __('laravel-cmf::generic.length') }}</th>
                    <th>{{ __('laravel-cmf::generic.not_null') }}</th>
                    <th>{{ __('laravel-cmf::generic.unsigned') }}</th>
                    <th>{{ __('laravel-cmf::generic.auto_increment') }}</th>
                    <th>{{ __('laravel-cmf::generic.index') }}</th>
                    <th>{{ __('laravel-cmf::generic.default') }}</th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
        ...
        <div v-else>
          <p>{{ __('laravel-cmf::database.table_no_columns') }}</p>
        </div>
    ...
    <div class="panel-footer">
        <input type="submit" class="btn btn-primary pull-right"
               value="@if($db->action == 'update'){{ __('laravel-cmf::database.update_table') }}@else{{ __('laravel-cmf::database.create_new_table') }}@endif"
...
@include('laravel-cmf::tools.database.vue-components.database-column')
@include('laravel-cmf::tools.database.vue-components.database-table-helper-buttons')
        ...
        methods: {
            addColumn(column) {
                ...
                if (column.name && this.hasColumn(column.name)) {
                    return toastr.error("{{ __('laravel-cmf::database.column') }} " + column.name + " {{ __('laravel-cmf::database.already_exists') }}");
                }
            ...
            renameColumn(column) {
                ...
                if ((existingColumn = this.getColumn(newName)) && (existingColumn !== column)) {
                    return toastr.error("{{ __('laravel-cmf::database.column') }} " + newName + " {{ __('laravel-cmf::database.already_exists') }}");
                }
            ...
            addIndex(index) {
                if (index.type == 'PRIMARY') {
                    if (this.table.primaryKeyName) {
                        return toastr.error("{{ __('laravel-cmf::database.table_has_index') }}");
                    }
