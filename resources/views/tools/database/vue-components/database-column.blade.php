...
<tr class="newTableRow">
    ...
    <td>
        <select :value="index.type" @change="onIndexTypeChange"
                ...
                class="form-control">
            <option value=""></option>
            <option value="INDEX">{{ __('laravel-cmf::database.index') }}</option>
            <option value="UNIQUE">{{ __('laravel-cmf::database.unique') }}</option>
            <option value="PRIMARY">{{ __('laravel-cmf::database.primary') }}</option>
        </select>
        <small v-if="column.composite" v-once>{{ __('laravel-cmf::database.composite_warning') }}</small>
    </td>
    ...
    <td>
        <div class="btn btn-danger delete-row" @click="deleteColumn"><i class="laravel-cmf-trash"></i></div>
    </td>
...
@include('laravel-cmf::tools.database.vue-components.database-types')
@include('laravel-cmf::tools.database.vue-components.database-column-default')
    ...
    Vue.component('database-column', {
        ...
        methods: {
            ...
            onIndexTypeChange(event) {
                if (this.column.name == '') {
                    return toastr.error("{{ __('laravel-cmf::database.name_warning') }}");
                }
