...
@if($db->action == 'update')
    @section('page_title', __('laravel-cmf::database.editing_table', ['table' => $db->table->name]))
@else
    @section('page_title', __('laravel-cmf::database.create_new_table'))
@endif
...
@section('page_header')
    <h1 class="page-title">
        ...
        @if($db->action == 'update')
            {{ __('laravel-cmf::database.editing_table', ['table' => $db->table->name]) }}
        @else
            {{ __('laravel-cmf::database.create_new_table') }}
        @endif
    </h1>
@stop
...
<ol class="breadcrumb hidden-xs">
    ...
    <li>
        <a href="{{ route('laravel-cmf.database.index') }}">
            {{ __('laravel-cmf::generic.database') }}
        </a>
    </li>

    @if($db->action == 'update')
    <li class="active">{{ __('laravel-cmf::generic.edit') }}</li>
    <li class="active">{{ $db->table->name }}</li>
    @else
    <li class="active">{{ __('laravel-cmf::generic.add') }}</li>
    @endif
</ol>
@endsection
...
@section('javascript')
    @include('laravel-cmf::tools.database.vue-components.database-table-editor')
