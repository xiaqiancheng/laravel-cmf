@if(isset($options->relationship))
    ...
    @if( !method_exists( $dataType->model_name, camel_case($row->field) ) )
        <p class="label label-warning"><i class="laravel-cmf-warning"></i> {{ __('laravel-cmf::form.field_select_dd_relationship', ['method' => camel_case($row->field).'()', 'class' => $dataType->model_name]) }}</p>
    @endif
    ...
    @if( method_exists( $dataType->model_name, camel_case($row->field) ) )
        ...
        <select class="form-control select2" name="{{ $row->field }}">
            ...
            @if(isset($options->options))
                <optgroup label="{{ __('laravel-cmf::generic.custom') }}">
            ...
            <optgroup label="{{ __('laravel-cmf::database.relationship.relationship') }}">
