...
LaravelCmf supports multiple languages for your models.To get started, you need to configure some things first.
...
```php
// with string
if (LaravelCmf::translatable(Post::class)) {
    // it's translatable
}

// with object of Model or Collection
if (LaravelCmf::translatable($post)) {
    // it's translatable
}
```
