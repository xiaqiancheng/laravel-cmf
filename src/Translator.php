...
class Translator implements ArrayAccess, JsonSerializable
{
    ...
    public function __construct(Model $model)
    {
        ...
        $this->model = $model;
        $this->locale = config('laravel-cmf.multilingual.default', 'en');
        $attributes = [];
    ...
    public function save()
    {
        ...
        foreach ($attributes as $key => $attribute) {
            if ($attribute['exists']) {
                ...
                $translation = LaravelCmfFacade::model('Translation')->where('table_name', $this->model->getTable())
            ...
            if (is_null($translation)) {
                $translation = LaravelCmfFacade::model('Translation');
            }
    ...
    protected function translateAttributeToOriginal($attribute)
    {
        $this->attributes[$attribute] = [
            ...
            'locale'   => config('laravel-cmf.multilingual.default', 'en'),
            'exists'   => true,
            'modified' => false,
        ];
    ...
    public function deleteTranslation($key)
    {
        ...
        LaravelCmfFacade::model('Translation')->where('table_name', $this->model->getTable())
            ->where('column_name', $key)
            ->where('foreign_key', $this->model->getKey())
            ->where('locale', $locale)
            ->delete();
    ...
    public function __call($method, array $arguments)
    {
        if (!$this->model->hasTranslatorMethod($method)) {
            throw new \Exception('Call to undefined method XADMIN\LaravelCmf\Translator::'.$method.'()');
        }
