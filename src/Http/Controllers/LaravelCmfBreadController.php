...
use XADMIN\LaravelCmf\Events\BreadUpdated;
use XADMIN\LaravelCmf\Facades\LaravelCmf;

class LaravelCmfBreadController extends Controller
{
    public function index()
    {
        ...
        return LaravelCmf::view('laravel-cmf::tools.bread.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create(Request $request, $table)
    {
        ...
        $dataType = LaravelCmf::model('DataType')->whereName($table)->first();
        ...
        return LaravelCmf::view('laravel-cmf::tools.bread.edit-add', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data = $res
                ? $this->alertSuccess(__('laravel-cmf::bread.success_created_bread'))
                : $this->alertError(__('laravel-cmf::bread.error_creating_bread'));
            if ($res) {
                event(new BreadAdded($dataType, $data));
            }

            return redirect()->route('laravel-cmf.bread.index')->with($data);
        } catch (Exception $e) {
            return redirect()->route('laravel-cmf.bread.index')->with($this->alertException($e, 'Saving Failed'));
        }
    }
    ...
    public function edit($table)
    {
        ...
        $dataTypeRelationships = LaravelCmf::model('DataRow')->where('data_type_id', '=', $dataType->id)->where('type', '=', 'relationship')->get();
        $scopes = [];
        if ($dataType->model_name != '') {
            $scopes = $this->getModelScopes($dataType->model_name);
        }

        return LaravelCmf::view('laravel-cmf::tools.bread.edit-add', compact('dataType', 'fieldOptions', 'isModelTranslatable', 'tables', 'dataTypeRelationships', 'scopes'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        /* @var \XADMIN\LaravelCmf\Models\DataType $dataType */
        try {
            $dataType = LaravelCmf::model('DataType')->find($id);
            ...
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('laravel-cmf::bread.success_update_bread', ['datatype' => $dataType->name]))
                : $this->alertError(__('laravel-cmf::bread.error_updating_bread'));
            if ($res) {
            ...
            return back()->with($this->alertException($e, __('laravel-cmf::generic.update_failed')));
        }
    }
    ...
    public function destroy($id)
    {
        ...
        $res = LaravelCmf::model('DataType')->destroy($id);
        $data = $res
            ? $this->alertSuccess(__('laravel-cmf::bread.success_remove_bread', ['datatype' => $dataType->name]))
            : $this->alertError(__('laravel-cmf::bread.error_updating_bread'));
        if ($res) {
        ...
        if (!is_null($dataType)) {
            LaravelCmf::model('Permission')->removeFrom($dataType->name);
        }
    ...
    public function addRelationship(Request $request)
    {
        ...
        try {
            ...
            $className = LaravelCmf::modelClass('DataRow');
            ...
            $newRow->order = intval(LaravelCmf::model('DataType')->find($request->data_type_id)->lastRow()->order) + 1;
    ...
    private function getRelationshipField($request)
    {
        ...
        $dataType = LaravelCmf::model('DataType')->find($request->data_type_id);
        ...
        $existingRow = LaravelCmf::model('DataRow')->where('field', '=', $relationshipField)->first();
        $index = 1;
    ...
    {
        LaravelCmf::model('DataRow')->destroy($id);
