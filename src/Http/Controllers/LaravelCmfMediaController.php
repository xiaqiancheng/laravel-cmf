...
use XADMIN\LaravelCmf\Events\MediaFileAdded;
use XADMIN\LaravelCmf\Facades\LaravelCmf;

class LaravelCmfMediaController extends Controller
{
    /** @var string */
    private $filesystem;
    ...
    {
        $this->filesystem = config('laravel-cmf.storage.disk');
    }
    ...
    public function index()
    {
        ...
        return LaravelCmf::view('laravel-cmf::media.index');
    }
    ...
    public function files(Request $request)
    {
        ...
        foreach ($storageItems as $item) {
            if ($item['type'] == 'dir') {
                ...
                if (empty(pathinfo($item['path'], PATHINFO_FILENAME)) && !config('laravel-cmf.hidden_files')) {
                    continue;
                }
                $files[] = [
                    'name'          => $item['basename'],
    ...
    public function new_folder(Request $request)
    {
        ...
        if (Storage::disk($this->filesystem)->exists($new_folder)) {
            $error = __('laravel-cmf::media.folder_exists_already');
        } elseif (Storage::disk($this->filesystem)->makeDirectory($new_folder)) {
            $success = true;
        } else {
            $error = __('laravel-cmf::media.error_creating_dir');
        }
        ...
        foreach ($request->get('files') as $file) {
            ...
            if ($file['type'] == 'folder') {
                if (!Storage::disk($this->filesystem)->deleteDirectory($file_path)) {
                    $error = __('laravel-cmf::media.error_deleting_folder');
                    $success = false;
                }
            } elseif (!Storage::disk($this->filesystem)->delete($file_path)) {
                $error = __('laravel-cmf::media.error_deleting_file');
                $success = false;
            }
        }
    ...
    public function rename(Request $request)
    {
        ...
        if (!Storage::disk($this->filesystem)->exists("{$location}/{$newFilename}")) {
            if (Storage::disk($this->filesystem)->move("{$location}/{$filename}", "{$location}/{$newFilename}")) {
                ...
                $error = __('laravel-cmf::media.error_moving');
            }
        } else {
            $error = __('laravel-cmf::media.error_may_exist');
        }
    ...
    public function upload(Request $request)
    {
        ...
        try {
            ...
            $allowedMimeTypes = config('laravel-cmf.media.allowed_mimetypes', '*');
            if ($allowedMimeTypes != '*' && (is_array($allowedMimeTypes) && !in_array($request->file->getMimeType(), $allowedMimeTypes))) {
                throw new Exception(__('laravel-cmf::generic.mimetype_not_allowed'));
            }
            ...
            $success = true;
            $message = __('laravel-cmf::media.success_uploaded_file');
    ...
    public function remove(Request $request)
    {
        ...
        try {
            ...
            if (!isset($data->{$field})) {
                throw new Exception(__('laravel-cmf::generic.field_does_not_exist'), 400);
            }

            if (@json_decode($multi)) {
                // Check if valid json
                if (is_null(@json_decode($data->{$field}))) {
                    throw new Exception(__('laravel-cmf::json.invalid'), 500);
                }
                ...
                if (is_null($key) || $key === false) {
                    throw new Exception(__('laravel-cmf::media.file_does_not_exist'), 400);
                }
            ...
            return response()->json([
               'data' => [
                   'status'  => 200,
                   'message' => __('laravel-cmf::media.file_removed'),
               ],
            ]);
        } catch (Exception $e) {
            $code = 500;
            $message = __('laravel-cmf::generic.internal_error');
    ...
    public function crop(Request $request)
    {
        ...
        try {
            ...
            $success = true;
            $message = __('laravel-cmf::media.success_crop_image');
        } catch (Exception $e) {
            $success = false;
            $message = $e->getMessage();
        }
