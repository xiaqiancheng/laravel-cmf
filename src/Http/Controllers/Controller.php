...
use XADMIN\LaravelCmf\Events\FileDeleted;
use XADMIN\LaravelCmf\Http\Controllers\ContentTypes\Checkbox;
use XADMIN\LaravelCmf\Http\Controllers\ContentTypes\Coordinates;
use XADMIN\LaravelCmf\Http\Controllers\ContentTypes\File;
use XADMIN\LaravelCmf\Http\Controllers\ContentTypes\Image as ContentImage;
use XADMIN\LaravelCmf\Http\Controllers\ContentTypes\MultipleCheckbox;
use XADMIN\LaravelCmf\Http\Controllers\ContentTypes\MultipleImage;
use XADMIN\LaravelCmf\Http\Controllers\ContentTypes\Password;
use XADMIN\LaravelCmf\Http\Controllers\ContentTypes\Relationship;
use XADMIN\LaravelCmf\Http\Controllers\ContentTypes\SelectMultiple;
use XADMIN\LaravelCmf\Http\Controllers\ContentTypes\Text;
use XADMIN\LaravelCmf\Http\Controllers\ContentTypes\Timestamp;
use XADMIN\LaravelCmf\Traits\AlertsMessages;
use Validator;
...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        if ($request->session()->has($slug.'_path') || $request->session()->has($slug.'_uuid')) {
            ...
            $data->save();
            if ($old_path != $new_path && !Storage::disk(config('laravel-cmf.storage.disk'))->exists($new_path)) {
                $request->session()->forget([$slug.'_path', $slug.'_uuid']);
                Storage::disk(config('laravel-cmf.storage.disk'))->move($old_path, $new_path);
                Storage::disk(config('laravel-cmf.storage.disk'))->deleteDirectory($folder_path);
            }
        }
    ...
    {
        if (Storage::disk(config('laravel-cmf.storage.disk'))->exists($path)) {
            Storage::disk(config('laravel-cmf.storage.disk'))->delete($path);
