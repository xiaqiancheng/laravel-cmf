...
namespace XADMIN\LaravelCmf\Traits;
...
use XADMIN\LaravelCmf\Translator;
...
trait Translatable
{
    ...
    {
        return $this->hasMany(LaravelCmf::model('Translation'), 'foreign_key', $this->getKeyName())
            ->where('table_name', $this->getTable())
            ->whereIn('locale', config('laravel-cmf.multilingual.locales', []));
    }
    ...
    public function getTranslationsOf($attribute, array $languages = null, $fallback = true)
    {
        if (is_null($languages)) {
            $languages = config('laravel-cmf.multilingual.locales', [config('laravel-cmf.multilingual.default')]);
        }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('laravel-cmf.multilingual.default'), false];
        }
        ...
        $default = config('laravel-cmf.multilingual.default');
    ...
    public function setAttributeTranslations($attribute, array $translations, $save = false)
    {
        ...
        $default = config('laravel-cmf.multilingual.default', 'en');
        $locales = config('laravel-cmf.multilingual.locales', [$default]);
    ...
    public function prepareTranslations(&$request)
    {
        ...
        foreach ($transFields as $field) {
            ...
            $request->merge([$field => $trans[config('laravel-cmf.multilingual.default', 'en')]]);
